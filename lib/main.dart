...
class _WorkspaceScreenState extends State<WorkspaceScreen> {
    ...
    setState(() {
      nodes.add(
        DeviceNode(
          ...
          name: template.name,
          position: const Offset(100, 100),
          ports: template.ports.map((p) => DevicePort(
  ...
  void _editDeviceTemplate(DeviceTemplate tpl) {
    if (nodes.any((n) => n.templateId == tpl.id)) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please delete all instances of this device from the canvas before editing/deleting.')),
      );
      return;
    }
  ...
  void _editCableType(CableType type) {
    if (cables.any((c) => c.type == type)) {
      ScaffoldMessenger.of(context).hideCurrentSnackBar();
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please delete all instances of this cable from the canvas before editing/deleting.')),
      );
      return;
    }
          ...
          builder: (context, setDialogState) {
            return AlertDialog(
                  ...
                  Row(
                    children: [
                      ...
                      DropdownButton<PortType>(
                        value: selectedType,
                        items: PortType.values.map((t) => DropdownMenuItem(value: t, child: Text(t.name))).toList(),
                  ...
                  Row(
                    children: [
                      ...
                      DropdownButton<PortGender>(
                        ...
                        items: PortGender.values.map((g) => DropdownMenuItem(value: g, child: Text(g.name))).toList(),
              ...
              actions: [
                ...
                ElevatedButton(
                  onPressed: () {
                    Navigator.pop(context, DevicePort(
                      id: DateTime.now().millisecondsSinceEpoch.toString(),
                      type: selectedType,
  ...
  Future<void> _showCustomDeviceDialog({DeviceTemplate? editingTemplate}) async {
    String name = editingTemplate?.name ?? '';
    List<DevicePort> ports = editingTemplate != null 
      ? editingTemplate.ports.map((p) => DevicePort(id: p.id, type: p.type, gender: p.gender, relativeCenter: p.relativeCenter)).toList() 
      : [];
            ...
            return AlertDialog(
                ...
                child: Column(
                  ...
                  children: [
                    TextField(
                      ...
                      decoration: const InputDecoration(labelText: 'Device Name (e.g., Target PC)'),
                      onChanged: (val) => name = val,
                    ),
                    const SizedBox(height: 16),
                    const Align(alignment: Alignment.centerLeft, child: Text('Ports:', style: TextStyle(fontWeight: FontWeight.bold))),
                    ...ports.asMap().entries.map((entry) {
                      ...
                      return ListTile(
                        ...
                        trailing: IconButton(
                          ...
                          onPressed: () => setDialogState(() => ports.removeAt(idx)),
                        ),
                      );
                    }),
              ...
              actions: [
                if (editingTemplate != null)
                  TextButton(
                    onPressed: () {
                      setState(() => customDevices.removeWhere((tpl) => tpl.id == editingTemplate.id));
                ...
                ElevatedButton(
                  onPressed: () {
                    final finalName = name.trim().isEmpty ? 'Custom Device' : name.trim();
                    final List<DevicePort> finalPorts = [];
                    for (int i = 0; i < ports.length; i++) {
                      finalPorts.add(DevicePort(
                        id: ports[i].id,
                        type: ports[i].type,
                        gender: ports[i].gender,
                        relativeCenter: Offset(i % 2 == 0 ? 0 : 140, 50.0 + (i ~/ 2) * 50.0),
                      ));
                    }
                    setState(() {
                      if (editingTemplate != null) {
                        final idx = customDevices.indexWhere((t) => t.id == editingTemplate.id);
                        if (idx != -1) customDevices[idx] = DeviceTemplate(id: editingTemplate.id, name: finalName, ports: finalPorts);
                      } else {
                        customDevices.add(DeviceTemplate(
                          id: DateTime.now().millisecondsSinceEpoch.toString(),
                          name: finalName,
                          ports: finalPorts,
                        ));
          ...
          builder: (context, setDialogState) {
            return AlertDialog(
              ...
              content: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    TextField(
                      controller: TextEditingController(text: name),
                      decoration: const InputDecoration(labelText: 'Cable Name (e.g., Custom USB)'),
                      onChanged: (val) => name = val,
                    ),
                    const SizedBox(height: 16),
                      ...
                      children: colors.map((c) => GestureDetector(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            border: selectedColor == c ? Border.all(color: Colors.blueAccent, width: 3) : Border.all(color: Colors.grey.shade400, width: 1),
                          ),
                        ),
                      )).toList(),
                    ),
              ...
              actions: [
                ...
                ElevatedButton(
                  onPressed: () {
                    final finalName = name.trim().isEmpty ? 'Custom Cable' : name.trim();
                    setState(() {
